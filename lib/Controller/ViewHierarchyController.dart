...
class ViewHierarchyController extends GetxController {
  ...
  Future<void> updatePostStatus(String postId, String status) async {
    DocumentReference jobPostRef = FirebaseFirestore.instance.collection("jobPost").doc(postId);
  ...
  Future<void> deletePostStatus(String postId) async {
    try {
      ...
      bool candidateExists = await jobPostRef.collection('candidate').limit(1).get().then((querySnapshot) => querySnapshot.docs.isNotEmpty);

      if (candidateExists) {
        // Delete all documents in the 'candidate' subcollection
        QuerySnapshot candidateSnapshot = await jobPostRef.collection('candidate').get();
  ...
  Future<List<Map<String, dynamic>>> fetchApplications(String postId) async {
    try {
      ...
      QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection("jobPost").doc(postId).collection("candidate").get();
  ...
  Future<List<DocumentSnapshot>> getAllApplicationData(DocumentReference userRef) async {
    try {
      // Fetch all documents in the "Application" collection
      QuerySnapshot querySnapshot = await userRef.collection("Application").get();
